"""
Module 40: seeds the GC_P7 (Gear Case, WAP-7, 3-Phase, M4-HR section) checksheet template.

Run once: `venv/bin/python scripts/seed_gear_case_p7_template.py`

Source: page 12 of the supplied WAP-7 checksheet - "Gear Case", 8 schedule activities repeated
once per gear case (Gear Case No.-1 through No.-6) - identical shape and standard values to the
WAG9HC Gear Case sheet, including item 8's gear case bolt property note (shown as a footnoted
"(8) G/case bolts" annotation on this sheet rather than a table row, but still the same 8th
checking point).
"""
...
def build(add):
    activities = [
        ...
        {"key": "gear_case_bolt_property", "label": "Property and Make of Gear Case Bolts (30x140-JEC 10.9, "
         "16x200-SEC 10.9, 12x30-SEC 8.8)", "field_type": "text", "required": True, "standard_value": "Noted"},
    ]
...
if __name__ == "__main__":
    run_seed(
        equipment_code="GC_P7", template_code="89", technology="3_PHASE",
        template_name="Checksheet for Gear Case (WAP-7)",
        description="Gear Case checksheet - WAP-7, 3-Phase - M4-HR section.",
        build_fn=build,
    )
