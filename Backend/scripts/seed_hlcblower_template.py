"""
Module 29.11: seeds the HLC Blower (Blower for Hotel Load Converter) checksheet template.

Run once: `venv/bin/python scripts/seed_hlcblower_template.py`

Bearing/end-cover Standard cells are blank on this sheet (plain number fields, no range). Run
Test has all three sub-groups (No Load/Full Load/Temp Rise) and the 3-lug check, matching the
blower family. Only "To check casing for any crack" (no separate MPT row). Adds a distinctive
"AIR FLOW" reading not present on any other sheet.
"""
...
def build(add):
    ...
    add_bearing_group(add, must_change, "must_change_bearing", "Bearing",
    ...
    add(
        "air_flow", field_label="Air Flow",
        field_type="number", required=False, unit="m/s",
    )


if __name__ == "__main__":
    run_seed(
        equipment_code="HLC Blower", template_code="11", technology="3_PHASE",
        template_name="Checksheet for HLC Blower",
        description="Blower for Hotel Load Converter checksheet.",
        build_fn=build,
    )
