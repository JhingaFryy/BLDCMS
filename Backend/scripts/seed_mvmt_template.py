"""
Seeds the MVMT (Blower Motor for Traction Motor) checksheet template from its physical reference
checksheet, reusing the exact same generic template engine, metadata conventions
(standard_value/authority_reference/breadcrumb grouping), and validation engine
(validation_rule/validation_threshold/negative_values) already built and verified for TMB/OCB -
nothing new is introduced here, this is data only.

Run once: `venv/bin/python scripts/seed_mvmt_template.py`

Equipment "MVMT" (id=15) and its Section M35-Aux <-> MVMT <-> Conventional SectionEquipmentMap
row already exist - only the ChecksheetTemplate + TemplateField rows are new. technology=
'CONVENTIONAL' matches Locomotive.technology's own format (the value Android/Dashboard actually
compare against when resolving which template applies) - this template must NOT resolve for
3-Phase locomotives, only Conventional ones.

MVMT's document is structurally almost identical to TMB's (same casing-crack+MPT split, same
4-sub-item Must Change Item incl. Imp Flakt/Arco, same "On Drum" rotor balancing note) - only the
bearing/end-cover dimension standards differ (65.002-65.015mm / 139.993-140.018mm, vs TMB's
60.002-60.015mm / 129.993-130.013mm). Built as its own independent field set rather than copying
TMB's rows, since the two are separate templates even though the pattern is shared.
...
def main():
    ...
    try:
        ...
        db.commit()
        print(f"MVMT template (id={template.id}) synchronized with reference checksheet.")
    except Exception:
        db.rollback()
        raise
    finally:
...
def get_or_create_template(db, equipment_id):
    ...
    template = ChecksheetTemplate(
        ...
        template_name="Checksheet for MVMT",
        description="Blower Motor for Traction Motor checksheet.",
        is_active=True,
    )
    db.add(template)
    db.flush()
...
def clear_existing_fields(db, template_id):
    ...
    while True:
        ...
        db.flush()
        removed += len(leaves)
    print(f"Removed {removed} pre-existing MVMT template field(s).")
...
def build_fields(db, template_id):
    ...
    # S.N. 6 - MVMT's own bearing dimension standard (65.002-65.015mm, not TMB's 60.002-60.015mm)
    bearing_seat = add(
    ...
    # S.N. 7 - MVMT's own end cover bore dimension standard (139.993-140.018mm)
    end_cover = add(
    ...
    # S.N. 17 - MVMT keeps the casing-crack row separate from MPT (matching TMB, unlike OCB/MVRH
    # which consolidate the two into one row).
    add(
    ...
    # S.N. 19 - matches TMB exactly: 4 generic reading boxes + an "On Drum" note.
    rotor_balancing = add(
        "rotor_balancing_portable_balancer", field_label="Rotor Balancing with Impeller by Portable Balancer",
        field_type="group", required=False,
        standard_value="Max. 15 micron on motor & 40 micron on casing", authority_reference="SMI-199",
    )
    for i in range(1, 5):
        add(
    ...
    add("rotor_balancing_on_drum", parent=rotor_balancing, field_label="On Drum", field_type="text", required=False)

    # S.N. 20 - Must Change Item: 4 sub-items, matching TMB's structure exactly (Bearing 6313,
    # Imp Flakt, Arco, transparent sleeve all present on this document, same PL numbers as TMB).
    must_change = add(
    ...
    impeller_flakt = add(
        "must_change_impeller_flakt", parent=must_change, field_label="Impeller Flakt (PL No. 23.44.9184)",
        field_type="group", required=False, authority_reference="R. Board's L/No. 2002/AC, II/L/10 dt. 24/05/06",
    )
    add("impeller_flakt_imp_no", parent=impeller_flakt, field_label="Imp. No.", field_type="text", required=False)
    add("impeller_flakt_mfd_date", parent=impeller_flakt, field_label="Mfd. Date", field_type="date", required=False)
    add("impeller_flakt_make", parent=impeller_flakt, field_label="Make", field_type="text", required=False)

    add(
        "must_change_arco", parent=must_change, field_label="Arco (PL No. 23.44.0405)",
    ...
    print(f"Inserted {order[0]} field(s) for the MVMT template.")
