"""
Module 40: seeds the TL (Traction Link / Push Pull Rod, WAG9HC, 3-Phase, M4-HR section) checksheet
template.

Run once: `venv/bin/python scripts/seed_traction_link_template.py`

Source: page 17 of the supplied WAG-9HC checksheet - "Traction Link (Push Pull Rod)", 6 schedule
activities repeated once per traction link (T/Link-1 through T/Link-4).
"""
...
def build(add):
    activities = [
        {"key": "flange_housing_rdpt", "label": "Check Traction Rod Flange and Housing Flange Against Crack by "
         "RDPT (At Bogie and Loco Body Side) (RDSO/SMI-259)", "field_type": "select",
...
if __name__ == "__main__":
    run_seed(
        equipment_code="TL", template_code="69", technology="3_PHASE",
        template_name="Checksheet for Traction Link",
        description="Traction Link (Push Pull Rod) checksheet - WAG9HC, 3-Phase - M4-HR section.",
        build_fn=build,
    )
