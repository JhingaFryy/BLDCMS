"""
Module 29.11: seeds the SIV Blower (Static Inverter for Vigilance blower motor) checksheet
template.

Run once: `venv/bin/python scripts/seed_sivblower_template.py`

Conventional-technology blower motor. Bearing 6205 and end-cover dimensions match Sc. MRB, but
this sheet is Conventional (not 3-Phase) and has full 3-part Run Test plus a casing-crack check
(no separate MPT of Impeller row).
"""
...
if __name__ == "__main__":
    run_seed(
        equipment_code="SIV Blower", template_code="19", technology="CONVENTIONAL",
        template_name="Checksheet for SIV Blower",
        description="Static Inverter for Vigilance blower motor checksheet.",
        build_fn=build,
    )
