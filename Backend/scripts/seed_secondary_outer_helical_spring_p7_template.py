"""
Module 40: seeds the SOHS_P7 (Secondary Outer Helical Spring, WAP-7, 3-Phase, M4-HR section)
checksheet template.

Run once: `venv/bin/python scripts/seed_secondary_outer_helical_spring_p7_template.py`

Source: page 19 of the supplied WAP-7 checksheet - "Secondary Outer Helical Spring" (CLW Drawing
No.-1209-01-015-010 & RDSO Drawing No.-SKVL-270), 8 schedule activities repeated once per wheel
number (only W.No.-3, 4, 9 and 10 carry a secondary spring), split by LH/RH.

Note: unlike WAG9HC, this sheet has NO Secondary INNER Helical Spring table at all - no such
checking point exists on the WAP-7 sheet, matching the M4-HR equipment mapping (there is no
SIHS_P7 equipment record). Only the secondary outer spring is checked for WAP-7.
"""
from _m4hr_template_helpers import add_final_remarks, run_seed

WHEEL_NUMBERS = [3, 4, 9, 10]
DRAWING_NO = "CLW Drawing No.-1209-01-015-010 & RDSO Drawing No.-SKVL-270"
    ...
    for w in WHEEL_NUMBERS:
        ...
        for side in ("LH", "RH"):
            ...
            add(f"sohs_w{w}_{side.lower()}_free_height", parent=s_group, field_label="Free Height",
                field_type="numeric_range", required=True, unit="mm", min_value=726.0, max_value=742.2,
            ...
            add(f"sohs_w{w}_{side.lower()}_working_load", parent=s_group, field_label="Spring Working Load",
                field_type="number", required=True, unit="Kg", standard_value="9867.5 Kg")
...
if __name__ == "__main__":
    run_seed(
        equipment_code="SOHS_P7", template_code="96", technology="3_PHASE",
        template_name="Checksheet for Secondary Outer Helical Spring (WAP-7)",
        description="Secondary Outer Helical Spring checksheet - WAP-7, 3-Phase - M4-HR section.",
        build_fn=build,
    )
